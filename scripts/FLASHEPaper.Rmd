...
# Results
## Participants
Table 1 displays the results of the descriptive characteristics of this sample. In total, there were `r format(nrow(data), big.mark = ",")` in the final analysis. Participants were evenly distributed by age and sex, with a mean age of `r mean_age` years. Adolescents' MVPA varied from as low as `r round(min_MVPA, 1)` to as high as `r round(max_MVPA, 1)`. Average weekly MVPA was `r avg_MVPA` minutes per day <!-- is that right? -->. The majority of participants reported attending a public school (*n* = `r pct_pub_school`).
...
# data <- data %>% 
#   mutate(
#     tsex <- as.factor(tsex), # sex
#     tethrac_rc_y <- as.factor(tethrac_rc_y), # race
#     tschltype <- as.factor(tschltype), # school type
#     tage_y <- as.factor(tage_y)
#   )
# 
# OR
# 
# data <- data %>% 
#   mutate(across(c(tsex, tethrac_rc_y, tschltype, tage_y), as.factor))
...
# 
# table_data <- data %>% 
#   select("Sex" = tsex, "School Type" = tschltype, "Race/Ethnicity" = tethrac_rc_y, "Age" = tage_y)
...
table_data2 %>%
  ...
  kableExtra::column_spec(1:5, width = "1.8cm") %>% 
  kableExtra::pack_rows("Another kind of school", 1, 2) %>%
  kableExtra::pack_rows("Home-Schooled", 3, 4) %>% 
  kableExtra::pack_rows("Private school", 5, 6) %>% 
  kableExtra::pack_rows("Public School", 7, 8) 
```
...
## Study Aim One
Regression results (presented in Table 3) were used to determine if there was a significant relationship with predicted MVPA during the week and sex, after adjusting for age and BMI. When sex was the only predictor in the model, there was no significant difference in the predicted MVPA of participants, _B_ = `r round(summary(model1)$coefficients[2, 1], 2)`, _SE_ = `r round(summary(model1)$coefficients[2, 2], 2)`, _t_(`r summary(model1)$df[2]`) = `r round(summary(model1)$coefficients[2, 3], 2)`, _p_ = `r round(summary(model1)$coefficients[2, 4], 2)`. Sex alone, explained only `r round(summary(model1)$r.squared*100, 2)`% of the variance in predicted MVPA, _F_(`r round(unname(summary(model1)$fstatistic))[2]`, `r round(unname(summary(model1)$fstatistic))[3]`) = `r round(unname(summary(model1)$fstatistic), 2)[1]`, _p_ = `r round(summary(model1)$coefficients[2, 4], 2)`.
