---
author: "Elizabeth Bates, Esmeralda Castro, & Zach Farley"
date: "12/8/2021"
output: pdf_document
bibliography: references.bib
title: "Patterns of Adolescent Physical Activity"
---
Liz: 
- Write 3rd paragraph in intro
- Methods Section (minus esmes part)
- Start results section with in-text code (N = , % boys/girls,  avg age, avg weekly MVPA) to describe summary table, and insert summary table.
- Data analysis section (short paragraph)
Esme:
- worked on intro/methods (done)
- Data analysis portion; methods and results
- Update figures/code chunks for MVPA
- change figure 1 colors from default
- Discussion Section **can someone share this task**
Zach:
- Write 2 Intro paragraphs (done)
- Preliminary methods sections (done)
- Abstract (after everyone else is done)
- Make bibliography
- In text citations
- Final Formatting


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE,
                      message = FALSE,
                      warning = FALSE,
                      error = FALSE,
                      fig.width = 8.5, 
                      fig.height = 8.5)

library(here)
library(rio)
library(janitor)
library(gtsummary)
library(tidyverse)
library(ggridges)
library(knitr) # can't remember if I used this or used it to try something

```


```{css, echo=FALSE}
h1 {
  text-align: center;
}
```

# Abstract
__ZACH WILL WRITE AT THE END__

# Introduction
  It is well known that regular physical activity (PA) is associated with a myriad of physical and mental health benefits. These benefits may be proximal in time to the performance of PA, such as reduced anxiety-like feelings, lower blood pressure, and enhanced cognitive function, or they may be more distal, such as increased strength and cardio-respiratory fitness, decreased depression-like symptoms, and long-term reduction of blood pressure _(Piercy KL, Troiano RP, Ballard RM, et al. The Physical Activity Guidelines for Americans. JAMA. 2018;320(19):2020–2028. doi:10.1001/jama.2018.14854)_. Regular PA is especially important for school-aged children (6-17 years old); PA has been shown to foster normal growth and development, improve mood and sleep quality, and greatly reduce the risk for developing chronic disease(s) as an adult _(Piercy KL, Troiano RP, Ballard RM, et al. The Physical Activity Guidelines for Americans. JAMA. 2018;320(19):2020–2028. doi:10.1001/jama.2018.14854)._ Aside from these physiological benefits, physically active students tend to have better educational outcomes (e.g., better grades, enhanced cognitive performance, and decreased behavioral problems) when compared to their less-active peers _(Centers for Disease Control and Prevention. The Association Between School-Based Physical Activity, Including Physical Education, and Academic Performance. Atlanta, GA; Centers for Disease Control and Prevention, US Department of Health and Human Services; 2010.)._ In addition, as both practice and research trends are leaning away from using body size, such as Body Mass Index (BMI), as a marker for physical health, they are turning to measures of regular physical activity and quantity of MVPA to indicate physical health (CITE).
  As outlined by the U.S. Department of Health and Human Services 2nd edition of the "Physical Activity Guidelines for Americans" (2018), the recommended daily dose of moderate- to vigorous-intensity physical activity (MVPA) for school-aged youth in the United States (US) is 60 minutes _(CITE https://health.gov/our-work/nutrition-physical-activity/physical-activity-guidelines/current-guidelines)_. Despite these recommendations regarding the benefits of meeting them, less than a quarter (24%) of school-aged children in the US achieve the recommended 60 minutes of MVPA a day _(The Child & Adolescent Health Measurement Initiative (CAHMI). 2016 National Survey of Childrens Health. Data Resource Center for Child and Adolescent Health;2016)._ **need to remove the citations that were removed from the intro I started**
  Many personal and environmental factors affect a child's ability to complete the recommended 60 minutes of MVPA a day. School-aged children spend nearly 3/4 of the weekdays in school, and yet there is a dearth of research addressing the association between school type (e.g. private, public, home-school) and health, specifically regarding physical activity. The link between type of school attended and physical activity needs to be examined due to the potential for program development and policy changes related to physical education class requirements in school. This paper seeks to address this gap in the literature by examining both patterns and quantity of physical activity according to demographics and type of school attended.
  _also let's talk about total MVPA, will mutate new total PA variabel_ -

## Research Questions

**1. How do patterns of physical activity for adolescents vary by sex after controlling for age and BMI?**

**2. Does minutes of physical activity performed during the week differ by the type of school attended (private, public, home, or other) **

## Hypotheses

**1. Patterns of physical activity of adolescents will significantly vary by sex after controlling for age and BMI.**

**2. There will be a significant association between the type of school attended at the weekday minutes of physical activity performed by adolescents.**

# Methods
## Data Source
This project uses data from the Family Life, Activity, Sun, Health and Eating (FLASHE) study. FLASHE data was collected through a cross-sectional design, using an internet-based survey _(CITE from https://cancercontrol.cancer.gov/brp/hbrb/flashe-study)._**Zach, I think you should cite the Oh 2017 article (in .bib file) for previous sentence**. Data collection occurred between April and October of 2014. The FLASHE study asked questions enabling researchers to examine correlates of cancer-related behaviors using behavioral measures of diet and physical activity, and other behaviors associated with cancer risk, such as sun-safety, sleep, and tobacco consumption. Sampling for the FLASHE study was drawn from a Consumer Opinion Panel and sampling design was intended to collect a representative sample reflective of demographics for the United States' general population (e.g., sex, education, income, age, region, household size). The conceptual model for the FLASHE study can be seen in Figure 1. It is important to note that data collection took place by sampling dyads of parent/caregiver-adolescent(ages 12-17). The FLASHE enrollment and design structure can be seen in Figure 2.

### Figure 1: FLASHE Conceptual Model **FYI Zach won't knit cuz of this. Maybe not in repo directory?**
![FLASHE Conceptual Model](/Users/zsfarley/Desktop/images/FLASHE_Concept.jpg)

### Figure 2: FLASHE Enrollment and Design
![FLASHE Enrollment and Design](/Users/zsfarley/Desktop/images/FLASHE_Enrollment.jpg)

## Sample Selection
In our study, we aim to examine patterns of adolescent **isn't this MVPA?->**PA, including when and how much **same here**PA they perform, and to explore the association between adolescents' weekly minutes of MVPA and their sex, after adjusting for age and body mass index (BMI), as well as explore the association between adolescent's school type with their exercise patterns. For the purpose of this study, our final sample will be composed of teenagers (13-17 years old) who gave responses for all physical activity and demographic variables. This process and final sample size is explained in the following sections.

## Statistical Analysis
_Demographic Characteristics_
Descriptive statistics were performed to determine the distribution of study variables among participants and to check for skewness and kurtosis. All analysis was conducted with the statistical software R [@R] **check if this worked in knit**.

_Study Aim One_
To address Study Aim 1, if there is an association between an adolescent's total MVPA and sex, after adjusting for age and BMI, a multiple regression was conducted. The dependent variable was an adolescent's total MVPA, which was calculated by adding the scores for 'MVPA during the weekday' and 'MVPA during the weekend'. The independent variable in the model was 'sex' (e.g., female and male). The variables included in the regression model that were controlled for were 'age' and 'BMI'. BMI was calculated from participants' self-reported height and weight with the equation: [weight (lb) / height (in) / height (in)] x 703 [@prevention2014calculating].

_Study Aim Two_
To address Study Aim 2, if there is an association between type of school attended and physical activity performed during the week, a one way ANOVA was conducted. The dependent variable was an adolescent's total MVPA, calculated as outlined above. The independent variables were the levels of 'school type' (e.g. public, private, homeschool or other). 

For both Study Aims 1 and 2 we created figures to visualize the data and results.

# Results

## Summary Table 1**remove the heading for final knit**
```{r summary-data-table-one}
data <- import(here("data", "final.csv")) %>% 
  as_tibble() %>% 
  clean_names()

# change chr to factors for our demographic vars
data$tsex <- as.factor(data$tsex) # sex
data$tethrac_rc_y <- as.factor(data$tethrac_rc_y) # race
data$tschltype <- as.factor(data$tschltype) # school type
data$tage_y <- as.factor(data$tage_y) # age

# Create df for table of demographic variables
table_data <- data %>% 
  select(c(13:15, 17))

colnames(table_data) <- c("Sex", "School Type",
                          "Race/Ethnicity", "Age")

table_data %>% 
  gtsummary::tbl_summary() %>% 
   as_gt() %>%
  gt::tab_header(title = "Table 1. Descriptive Characteristics")
```
```{r}

```


```{r echo=FALSE}

count <- data %>% count()

data$total_pa_wk_min <- as.vector(data$total_pa_wk_min)

max_MVPA <- data %>% 
  summarize(max_MVPA = max(total_pa_wk_min))

min_MVPA <- data %>% 
  summarize(min_MVPA = min(total_pa_wk_min))

mean_age <- data %>%
  summarize(mean_age = round(mean(tage_x +10), digits=1))

mean_MVPA <- data %>%
  group_by(tschltype) %>%
  summarize(mean_MVPA = mean(total_pa_wk_min))

#I keep getting this error and not sure why :1: In mean.default(total_pa_wk_min) :argument is not numeric or logical: returning NA 
            




  #Found a reference for how to create inline code from the gtsummary table but not working for me yet.. stay tuned
  #Esme - can you help me add an item to the table of median MVPA?
  
```


## Participants
Table 1 displays the results of the descriptive characteristics of this sample. In total, there were `r count` in the final analysis. Participants were evenly distributed by age and sex, with a mean age of `r mean_age`. Adolescents' MVPA varied from as low as `r min_MVPA` to as high as `r max_MVPA`. Average weekly MVPA was `r mean_MVPA` **when i figure out how to get it calculated and saved as an object correctly**. The majority of participants reported attending a public school `r inline_text(table_data, variable = School Type, level = "Public School", column = "N=1,192"`
**leaving this in until we figure out if it knits: in-line?** (85.99%).


##Patterns of Physical Activty by School Type
Table 2 displays the the amount of weekly physical activity and when it was completed (during the week or on the weekend) across school types with respect to sex. Average BMI per school type and sex is also included.

## Summary Table 2 **remove the heading for final knit**
```{r}
summary_table1 <- data %>%
  group_by(tschltype, tsex) %>%
  summarize(mean_age=mean(tage_x + 10), mean_bmi= mean(bmi), sd_bmi=sd(bmi), mean_total_pa_wk_min = mean(total_pa_wk_min), sd_total_pa_wk_min=sd(total_pa_wk_min), mean_weekofs= mean(xtpredweekofs), sd_weekofs= sd(xtpredweekofs), mean_weekw= mean(xtpredweekw), sd_weekw=sd(xtpredweekw)) 

summary_table1 %>%
  kable(caption = "Table 2. Pattern of Physical Activity by School Type",
        col.names = c("School Type", "Sex", "Mean age", "Mean BMI", "SD of BMI", "Mean Weekly Minutes of PA", "SD of Weekly Minutes of PA", "Mean minutes of PA during the Week", "SD of minutes of PA during the Week", "Mean minutes of PA on weekend", "SD of minutes of PA on weekend", ), 
        digits = 2)

```


Participants' average MVPA by age is displayed in Figure 3. Patterns were observed with decreases in the predicted MVPA as participants got older. Male participants were predicted to obtain a higher average of MVPA than their female counterparts, but not by much more.

## MVPA by sex after controlling for age and bmi
```{r PA-BMI-race-ethnicity}
data %>% 
  group_by(tage_y, tsex) %>% 
  summarize(mean_mvpa = mean(total_pa_wk_min)) %>% 
  ggplot(aes(tsex, mean_mvpa)) +
  geom_col(aes(fill = tsex), position = "dodge") +
  facet_wrap(~tage_y) +
  scale_fill_brewer(palette = 2) +
  theme_minimal() +
  labs(y = "Predicted Average MVPA (min/week)",
       x = "Sex",
       title = "Physical Activity by Sex",
       subtitle = "with Age and BMI",
       tag = "Figure 3.") +
  theme(legend.title = element_blank()) +
  theme(axis.text.x = element_blank())
```

## Study Aim One
Regression results (presented in Table 3) **if can make one** were used to determine if there was a significant relationship with predicted MVPA during the week and sex, after adjusting for age and BMI. When sex was the only predictor in the model, there was no significant difference in the predicted MVPA of participants, B = -8.42, SE = 4.74, t(1190) = -1.78, p = .08. Sex alone, explained only 0.26% of the variance in predicted MVPA, F(1, 1190) = 3.16, p = .08.

When age and bmi were adjusted for in the regression model, there was a significant difference in the relationship between sex and predicted MVPA during the week. Female adolescents engaged in 10.06 minutes less MVPA during the week than male participants, SE = 3.35, t(1185) = -3.00, p < .001.

```{r regression-code}
# regression without controlled variables of age and bmi (Model 1)
# Make "Male" the reference group in predictor variable: tsex
data$tsex <- relevel(data$tsex, ref = "Male")
contrasts(data$tsex)
model1 <- lm(total_pa_wk_min ~ 1 + tsex,
            data = data)
summary(model1)

# final regression model with all variables (Model 2)
model2 <- lm(total_pa_wk_min ~ 1 + tsex + tage_y + bmi,
            data = data)
summary(model2)
confint(model2)

model2 %>% 
  tbl_regression()
# couldn't figure out how to add a header here
```

## Study Aim Two
**figure below should work with these results**
An ANOVA was used to determine if there was a significant association between type of school attended and amount of physical activity performed during the week. The results indicate that there is no significant association. Figure 4 provides a visualization of the associations between the variables of interest (School Type and Predicted Weekly MVPA)

<!-- Figures to be inserted under text they belong -->
## MVPA by School Type
```{r PA-by-school-type}

data %>% 
  ggplot(aes(total_pa_wk_min, tschltype)) +
  geom_density_ridges(aes(fill = factor(tsex)),
                      alpha = 0.7) +
  scale_fill_viridis_d(option = "magma", name = "Sex") +
  theme_ridges() +
  labs(title = "Total Physical Activity by School Type",
       y = "School Type",
       x = "Predicted Weekly MVPA (min)",
       tag = "Figure 4.")
```

# Discussion
<!-- 2-3 paragraphs -->

Based on our analysis of adolescents who participated in the FLASE study...
Implications for programs to promote physical activity in or out of school..
Implications for policy..
Next steps for research include examining what motivates adolescents to complete physical activity, and how their environment moderates or mediates the amount of physical activity they complete.

**Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plots and tables**

\newpage
# References